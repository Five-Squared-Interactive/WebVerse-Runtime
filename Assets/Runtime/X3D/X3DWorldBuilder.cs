...
namespace X3D
{
    ...
    public class X3DWorldBuilder
    {
        ...
        private void ProcessNode(X3DNode node, X3DEntityHandle parent)
        {
            ...
            // Handle USE references
            if (node.Attributes.TryGetValue("USE", out string useRef) && defToEntity.ContainsKey(useRef))
            {
                // USE means reference an existing DEF - we might need to clone or reference
                // For now, skip since the entity already exists
                return;
            }
            ...
            // Store DEF reference
            if (entityHandle != null && !string.IsNullOrEmpty(defName))
            {
        ...
        private X3DEntityHandle ProcessShape(X3DNode node, X3DEntityHandle parent)
        {
            ...
            // Extract mesh and material from the temporary GameObject
            ...
            if (shapeComponent != null)
            {
                ...
                if (material == null && shapeComponent.appearance != null)
                {
                    MeshRenderer mr = shapeComponent.appearance.GetComponent<MeshRenderer>();
                    if (mr != null) material = mr.sharedMaterial;

                    // Check for X3DMaterial component
                    var matComp = shapeComponent.appearance.GetComponent<X3DMaterialComponent>();
                    if (matComp != null)
                    {
            ...
            if (mesh != null)
            {
                handle = adapter.CreateMeshEntity(parent, mesh, material, Vector3.zero, Quaternion.identity, Vector3.one, tag);
            }
            else
            {
        ...
        private void ProcessBackground(X3DNode node)
        {
            ...
            settings.SkyColor = Color.black;
            settings.GroundColor = Color.black;

            if (node.Attributes.TryGetValue("skyColor", out string skyColorStr))
            {
                // Can be multiple colors; use first
                var colors = ParseColorArray(skyColorStr);
                if (colors.Length > 0) settings.SkyColor = colors[0];
            }

            if (node.Attributes.TryGetValue("groundColor", out string groundColorStr))
            {
                var colors = ParseColorArray(groundColorStr);
                if (colors.Length > 0) settings.GroundColor = colors[0];
            }
