        ...
        public static GameObject X3DNodeToGameObject(X3DNode node)
        {
            ...
            switch (node.Name)
            {
                ...
                case "GeoElevationGrid":
                    ...
                    foreach (var child in node.Children)
                    {
                        if (child.Name == "GeoOrigin") geoElev.geoOrigin = X3DNodeToGameObject(child);
                        if (child.Name == "Color") geoElev.color = X3DNodeToGameObject(child);
                        if (child.Name == "Normal") geoElev.normal = X3DNodeToGameObject(child);
                        if (child.Name == "TextureCoordinate") geoElev.texCoord = X3DNodeToGameObject(child);
                        if (child.Name == "MetadataString" && child.Attributes.TryGetValue("value", out string metaStr))
                ...
                case "Cone":
                    ...
                    // Adjust scale to approximate a cone (scale Y, set top radius to 0)
                ...
                case "Sphere":
                    ...
                    if (node.Attributes.TryGetValue("radius", out string radiusStr) && float.TryParse(radiusStr, out float radius))
                    {
                ...
                case "LOD":
                    ...
                    foreach (var child in node.Children)
                    {
                        if (child.Name == "MetadataString" && child.Attributes.TryGetValue("value", out string metaStr))
                            ...
                            levels.Add(X3DNodeToGameObject(child));
                    }
