...
namespace FiveSQD.WebVerse.Handlers.X3D
{
    ...
    public class X3DHandler : BaseHandler
    {
        ...
        /// <summary>
        /// Get the title from an X3D document's WorldInfo node.
        /// </summary>
        /// <param name="resourceURI">URI of the X3D file.</param>
        /// <param name="onComplete">Callback with the title string (or "X3D World" if not found).</param>
        public void GetX3DTitle(string resourceURI, Action<string> onComplete)
        {
            StartCoroutine(GetTitleCoroutine(resourceURI, onComplete));
        }

        private IEnumerator GetTitleCoroutine(string resourceURI, Action<string> onComplete)
        {
            string x3dContent = null;
            bool isRemote = resourceURI.StartsWith("http://") || resourceURI.StartsWith("https://");
            ...
            if (isRemote)
            {
                ...
                if (!downloadComplete || string.IsNullOrEmpty(x3dContent))
                {
                    onComplete?.Invoke("X3D World");
                    yield break;
                }
#else
                onComplete?.Invoke("X3D World");
            ...
            {
                // Resolve the file path (same logic as LoadFromLocal)
                string resolvedPath = resourceURI;
                ...
                // Check if this is a relative path that needs resolution via fileHandler
                if (runtime != null && runtime.fileHandler != null)
                {
                    string fileHandlerPath = Path.Combine(runtime.fileHandler.fileDirectory,
            ...
            // Parse and find WorldInfo title
            string title = ExtractTitleFromX3D(x3dContent);
            onComplete?.Invoke(title);
        }

        /// <summary>
        /// Extract title from X3D content by finding it in head/meta or WorldInfo node.
        /// </summary>
        private string ExtractTitleFromX3D(string x3dContent)
        {
            if (string.IsNullOrEmpty(x3dContent))
                return "X3D World";
            ...
            // First try to find title in head/meta (X3D 3.x+ style)
            string title = FindMetaTitle(rootNode);
            if (!string.IsNullOrEmpty(title))
                return title;
            
            // Fall back to WorldInfo title attribute
            title = FindWorldInfoTitle(rootNode);
            return string.IsNullOrEmpty(title) ? "X3D World" : title;
        }

        /// <summary>
        /// Find title from head/meta element.
        /// </summary>
        private string FindMetaTitle(X3DNode node)
        {
            if (node == null)
                return null;
            ...
            if (node.Name == "head")
            {
                foreach (var child in node.Children)
                {
                    if (child.Name == "meta")
                    {
                        // Check if this meta has name="title"
                        if (child.Attributes.TryGetValue("name", out string name) && 
                            name.Equals("title", StringComparison.OrdinalIgnoreCase))
                        {
                            if (child.Attributes.TryGetValue("content", out string content))
                                return content;
                        }
                    }
                }
            ...
            foreach (var child in node.Children)
            {
                string found = FindMetaTitle(child);
        ...
        /// <summary>
        /// Recursively find WorldInfo node and extract title.
        /// </summary>
        private string FindWorldInfoTitle(X3DNode node)
        {
            if (node == null)
                return null;
            ...
            {
                if (node.Attributes.TryGetValue("title", out string title))
                    return title;
            }

            foreach (var child in node.Children)
            {
                string found = FindWorldInfoTitle(child);
                if (!string.IsNullOrEmpty(found))
                    return found;
            }
